// app.js (no-build React)
// VIEWER:    index.html  -> window.__MODE__='view'  (read-only)
// MANAGER:   edit.html   -> window.__MODE__='edit'  (edit + save)
...
function fmtDay(base,off){ const d=new Date(base); d.setDate(d.getDate()+off); return d.toLocaleDateString(undefined,{month:'short',day:'numeric'}); }
function nextMondayISO(iso){ const d=new Date(iso); d.setDate(d.getDate()+7); return d.toISOString().slice(0,10); }
function safeName(name){ return (name||'').replace(/\s\d{4,6}$/, ''); } // HIDE trailing 5–6 digit IDs
...
function parseHours(label){
  ...
  const P = t => {
    ...
    let h=+m[1]; const half=m[2]?0.5:0; const ap=m[3];
...
// 8AM–8PM; 30min steps; lengths 6–11.5h, plus ONLY 8AM–8PM for 12h
function generateShiftOptions(){
  const opts=['Off']; const step=0.5;
  const toLabel=h=>{ const t=Math.round(h*60); const H=Math.floor(t/60), m=t%60; const ap=H<12?'AM':'PM'; let hh=H%12; if(hh===0) hh=12; return `${hh}${m?':30':''}${ap}`; };
  for(let s=8; s<=14; s+=step){
    for(let L=6; L<=12; L+=step){
      ...
      const label = `${toLabel(s)}–${toLabel(e)}`; if(!opts.includes(label)) opts.push(label);
    }
  }
  return opts;
}
...
function useQueryWeek(){
  ...
  const base = param ? new Date(param) : startOfWeek(new Date());
  const weekISO = startOfWeek(base).toISOString().slice(0,10);
  return [weekISO, (delta)=>{
    const d=startOfWeek(new Date(weekISO));
...
function App(){
  ...
  // editor password from env via api
  React.useEffect(()=>{
    if(MODE==='edit'){
  ...
  const setShift = (empId, dayKey, val) => {
    setData(prev => ({...prev, schedule:{...prev.schedule, [empId]:{...(prev.schedule[empId]||{}), [dayKey]:val}}}));
  };

  async function saveCurrent(targetWeekISO){
    setSaving(true);
    try{
      const payload={ weekISO: targetWeekISO || weekISO, data:{...data, weekStart: targetWeekISO || weekISO} };
      const res = await fetch('/api/save', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
      if(!res.ok){ const t=await res.text(); throw new Error(t||'Save failed'); }
      alert('Saved!');
      if(targetWeekISO && targetWeekISO!==weekISO){ window.location.href=`?week=${targetWeekISO}`; }
    }catch(e){ alert('Error: '+e.message); } finally{ setSaving(false); }
  }

  function duplicateToNextWeek(copyShifts){
    const nextISO = nextMondayISO(weekISO);
    const nextData = {...data, weekStart: nextISO, schedule:{}};
    for(const e of employees){
      nextData.schedule[e.id] = {};
      for(const d of days){
        nextData.schedule[e.id][d.key] = copyShifts ? (schedule[e.id]?.[d.key] || 'Off') : 'Off';
      }
    }
    setData(nextData);
    saveCurrent(nextISO);
      ...
      <div className="container">
        <div className="bar">
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:'#64748b'}}>Week of {weekDate.toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'})}</div>
          </div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    {days.map(d=> <th key={d.key}>{d.label} <span style={{color:'#94a3b8',fontSize:12}}>{fmtDay(weekISO,d.off)}</span></th>)}
                ...
                <tbody>
                  {employees.map((e,idx)=>(
                    <tr key={e.id} style={{background:idx%2?'#f8fafc':'#fff'}}>
                      <td className="sticky-left" style={{padding:12,borderTop:'1px solid #f1f5f9',fontWeight:600}}>{safeName(e.name)}</td>
                      {days.map(d=>(
                        <td key={d.key}>
                          {canEdit ? (
                            <select value={schedule[e.id]?.[d.key]||'Off'} onChange={ev=>setShift(e.id,d.key,ev.target.value)} style={{padding:'6px 8px',border:'1px solid #e5e7eb',borderRadius:8}}>
          ...
          <div style={{display:'grid',gap:12,marginTop:16}}>
            {employees.map(e=>(
              <div key={e.id} className="card">
                <div className="head">{safeName(e.name)}</div>
                <div className="row">
                  <div>
                    <div style={{color:'#64748b',fontSize:13}}>Shift</div>
                    {canEdit ? (
                      <select value={schedule[e.id]?.[active]||'Off'} onChange={ev=>setShift(e.id,active,ev.target.value)} style={{padding:'6px 8px',border:'1px solid #e5e7eb',borderRadius:8}}>
                        {SHIFT_OPTIONS.map(opt=><option key={opt} value={opt}>{opt}</option>)}
                      </select>
                    ) : (
            ...
            <div style={{textAlign:'right',color:'#475569',fontSize:14,paddingBottom:24}}>
              Day total: <span style={{fontWeight:700}}>{employees.reduce((acc,e)=> acc+parseHours(schedule[e.id]?.[active]),0)}</span>
            </div>
          </div>
        )}
